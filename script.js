...
menuItems.forEach((item, index) => {
  item.style.animationDelay = `${(totalDelay * index) / (menuItems.length - 1)}s`
})
...
const initializeVisitSection = () => {
  ...
  const observeVisitTitle = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
...
const initializeParkSections = () => {
  const sectionContainers = document.querySelectorAll('.rail-section-container');
  const sectionDescription = document.querySelector('.rail-section-description');
  ...
  window.addEventListener('scroll', () => {
    if (sectionDescription && window.innerWidth > 970) {
      ...
      if (windowHeight - rect.top > 0 && rect.bottom > 0) {
        ...
        sectionDescription.style.transform = `translateY(${(windowHeight - rect.top) * relVelocity}px)`;
      }
    }
  });
  ...
  sectionContainers.forEach(container => {
    const videoElement = container.querySelector('.hover-media');
